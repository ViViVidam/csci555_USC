...
#ifndef THANOS_NIMAR_HPP
#define THANOS_NIMAR_HPP
...
#include "migration/strategies/thread_strategy.hpp" // for tickets_node
...
namespace migration::thread {
	class nimar : public Istrategy {
	private:
		static constexpr bool EVOLVE_TICKETS = false;

		static constexpr bool NODE_MIGRATION = true;
		...
		[[nodiscard]] auto get_migration(const pid_t tid) const -> std::vector<migration_cell> {
			...
			const auto src_node  = system_info::pinned_node_from_tid(tid);
			const auto curr_pid  = system_info::pid_from_tid(tid);
			const auto curr_perf = perf_table.performance(tid);

			const auto src_tickets = tickets_node(tid, src_node);
			...
			for (const auto dst_node : system_info::nodes_by_distance(src_node)) {
				...
				simple_migration_cell mc(tid, curr_pid, dst_node, src_node, true, curr_perf);

				const auto dst_tickets = tickets_node(tid, dst_node);

				// Free core: possible simple migration with a determined score
				if (system_info::is_cpu_free_in_node(dst_node) && dst_tickets > src_tickets) {
					migration_list.emplace_back(std::initializer_list<simple_migration_cell>{ mc }, dst_tickets);
					continue;
				}

				// Not a node with free cores: get its TIDs info so a possible interchange can be planned
				// We will choose the TID which generates the highest number of tickets
				const auto swap_tid_tickets_optional = best_pid_for_swap_node(src_node, dst_node, migrated_pids_);
				...
				const auto current_tickets   = src_tickets + tickets_node(swap_tid, dst_node);
				...
				if (estimated_tickets > current_tickets) {
					...
					simple_migration_cell mc_2(swap_tid, swap_pid, src_node, dst_node, NODE_MIGRATION, swap_perf);
...
#endif /* end of include guard: THANOS_NIMAR_HPP */
