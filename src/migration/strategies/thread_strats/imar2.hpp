...
#ifndef THANOS_IMAR2_HPP
#define THANOS_IMAR2_HPP
...
#include "migration/strategies/thread_strategy.hpp" // for tickets_cpu, und...
#include "migration/tickets.hpp"                    // for PERF_THRESHOLD
#include "migration/utils/times.hpp"                // for time_go_up, time...
...
namespace migration::thread {
	...
	private:
		...
		[[nodiscard]] auto get_migration(const pid_t tid) -> std::vector<migration_cell> {
			...
			for (const auto dst_node : system_info::nodes_by_distance(src_node)) {
				...
				for (const auto & dst_cpu : system_info::cpus_from_node(dst_node)) {
					...
					// We will choose the TID which generates the higher number of tickets
		...
		auto get_migration_list(const std::vector<std::pair<real_t, pid_t>> & worst_tids)
		...
		auto perform_migration_algorithm() -> std::vector<migration_cell> {
			...
			for (const auto & [tid, data] : perf_table) {
				...
				if (perf_table.is_running(tid) && perf < PERF_THRESHOLD &&
	...
	public:
		...
		void migrate() override {
			...
			real_t last_migr_profit = compare_last_migrations(verbose::print_with_lvl(verbose::DEFAULT_LVL));
			...
			const auto diff = total_performance / last_performance;
			...
			if (diff <= 0) { // First time or no data...
				// So we do nothing...
				return;
			}

			if (!last_migrations.empty() && (diff < UNDO_THRESHOLD || (last_migr_profit < 0 && diff < 1.0))) {
				// We are doing MUCH worse or last migrations were not successfully, let's undo
				time_go_up();
				undo_last_migrations(true);

				return;
			}

			if (diff < 1.0) { // We are doing a bit worse
				time_go_up();
			} else { // We are doing better
				time_go_down();
			}
...
#endif /* end of include guard: THANOS_IMAR2_HPP */
