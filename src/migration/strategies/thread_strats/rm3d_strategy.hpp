...
#ifndef THANOS_RM3D_STRATEGY_HPP
#define THANOS_RM3D_STRATEGY_HPP
...
namespace migration::thread {
	class rm3d_strategy : public Istrategy {
	private:
		...
		template<class T>
		static inline auto is_better(const T current, const T best) -> bool {
			return current > best;
		}

		template<class T>
		[[nodiscard]] static inline auto probability(const T current, const T best, const real_t temperature) {
			const T probability = (T(1.0) - (best - current) / best) * temperature;
		...
		[[nodiscard]] static auto optimal_schedule(const mapping::schedule_t & init_solution) -> mapping::schedule_t {
			...
			const auto init_perf = init_solution.expected_perf();
			auto       curr_perf = init_perf;
			auto       best_perf = init_perf;

			size_t iter             = 0;
			size_t iter_without_imp = 0;

			auto temperature = INITIAL_TEMP;

			if (verbose::print_with_lvl(verbose::LVL4)) {
				std::cout << "Iter " << iter << ". Initial performance: " << utils::string::to_string(init_perf)
				          << '\n';
			}
			...
			for (iter = 0; iter < MAX_ITER && iter_without_imp < MAX_ITER_WO_IMPR; ++iter) {
				...
				const auto cand_perf     = cand_solution.expected_perf();

				if (verbose::print_with_lvl(verbose::LVL4)) {
					const auto improvement_to_curr = (cand_perf - curr_perf) / curr_perf;
					const auto improvement_to_best = (cand_perf - best_perf) / best_perf;

					std::cout << "Iter " << iter << ". Candidate performance: " << utils::string::to_string(cand_perf)
					          << ". Current: " << utils::string::to_string(curr_perf)
					          << " (Impr: " << utils::string::to_string(improvement_to_curr) << "%)"
					          << ". Best: " << utils::string::to_string(best_perf)
					          << ". (Impr: " << utils::string::to_string(improvement_to_best) << "%)" << '\n';
				}

				// Accept the candidate solution
				if (is_better(cand_perf, curr_perf)) {
					if (verbose::print_with_lvl(verbose::LVL4)) {
						std::cout << "Iter " << iter
						          << ". New target performance: " << utils::string::to_string(cand_perf)
						          << ". Old: " << utils::string::to_string(curr_perf) << '\n';
					}
					...
					if (is_better(cand_perf, best_perf)) {
						if (verbose::print_with_lvl(verbose::LVL4)) {
							std::cout
							    << "Iter " << iter
							    << ". New best solution found with performance: " << utils::string::to_string(cand_perf)
							    << ". Old: " << utils::string::to_string(best_perf) << '\n';
						}
				...
				} else if (utils::arithmetic::rnd(0.0, 1.0) < probability(cand_perf, curr_perf, temperature)) {
					++iter_without_imp;
					...
					if (verbose::print_with_lvl(verbose::LVL4)) {
						std::cout
						    << "Iter " << iter
						    << ". Accepted worst solution. New performance: " << utils::string::to_string(cand_perf)
						    << ". Old: " << utils::string::to_string(curr_perf) << '\n';
					}
			...
			const auto improvement = (best_perf - init_perf) / init_perf * 100;

			if (verbose::print_with_lvl(verbose::DEFAULT_LVL)) {
				const auto diff = best_perf - init_perf;
				std::cout << "Iter " << iter
				          << ". Initial solution performance: " << utils::string::to_string(init_perf)
				          << ". New: " << utils::string::to_string(best_perf)
		...
		[[nodiscard]] static auto get_migration_list(const std::vector<mapping::thread_t> & threads)
			...
			const auto init_perf = init_solution.expected_perf();
			const auto best_perf = best_solution.expected_perf();

			const auto improvement = (best_perf - init_perf) / init_perf * real_t(100);
	...
	public:
		...
		void migrate() override {
			...
			const auto total_performance = perf_table.total_performance();
...
#endif /* end of include guard: THANOS_RM3D_STRATEGY_HPP */
